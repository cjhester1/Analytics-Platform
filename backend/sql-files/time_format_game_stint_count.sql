...
stint_counter AS (
    SELECT 
        ...
        pn.period,
        pn.player_name,
        ROW_NUMBER() OVER (PARTITION BY pn.game_id, pn.team_id, pn.player_name ORDER BY pn.period) AS stint_number,  -- Rename total_stints to stint_number
...
SELECT 
    ...
    sc.player_name,
    sc.period,
    sc.stint_number,  -- Place stint_number after period
    sc.stint_start_time,
    sc.stint_end_time
